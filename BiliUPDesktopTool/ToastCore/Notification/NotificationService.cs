...
namespace ToastCore.Notification
{
    ...
    public class NotificationService : NotificationActivator
    {
        ...
        /// <param name="slience">是否静音</param>
        /// <param name="commands">自定义命令组</param>
        public void Notify(string title, string content, string tag, bool slience = false, params ToastCommands[] commands)
        {
            XmlDocument xml = ToastNotificationManager.GetTemplateContent(ToastTemplateType.ToastText02);
            AddTitle(xml, title, content);
            AddCommands(xml, commands);
            if (slience) SetSlience(xml);
            ShowToast(xml, tag);
        }
        ...
        /// <param name="content">文本</param>
        /// <param name="commands">自定义命令组</param>
        public void Notify(Uri picuri, string title, string content, params ToastCommands[] commands)
        {
            XmlDocument xml = ToastNotificationManager.GetTemplateContent(ToastTemplateType.ToastText02);
            AddTitle(xml, title, content);
            AddBigLogo(xml, picuri.ToString());
            AddCommands(xml, commands);
            ShowToast(xml);
        }
        ...
        protected static void AddCommands(XmlDocument xml, params ToastCommands[] commands)
        {
            ...
            foreach (var command in commands)
            {
                ...
                if (string.IsNullOrEmpty(command.ActivationType)) action.SetAttribute("activationType", "foreground");
                else action.SetAttribute("activationType", command.ActivationType);
                action.SetAttribute("arguments", command.Argument);
                action.SetAttribute("content", command.Content);
                if (!string.IsNullOrEmpty(command.AfterActivationBehavior)) action.SetAttribute("afterActivationBehavior", command.AfterActivationBehavior);
        ...
        /// 发送通知
        /// </summary>
        protected static void ShowToast(XmlDocument xml, string tag)
        {
            ToastNotification toast = new ToastNotification(xml)
            {
                Tag = tag
        ...
        /// </summary>
        /// <param name="xml"></param>
        private void SetSlience(XmlDocument xml)
        {
            XmlElement audio = null;
            if (xml.GetElementsByTagName("audio").Count != 0)
            {
                audio = (XmlElement)xml.GetElementsByTagName("audio")[0];
            ...
            else
            {
                ...
                audio.SetAttribute("silent", "true");
                ((XmlElement)xml.GetElementsByTagName("toast")[0]).AppendChild(audio);
            }
        }
